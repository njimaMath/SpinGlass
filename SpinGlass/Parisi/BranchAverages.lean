...
(`branchMark`), the interpolating Hamiltonian `√t H_N(σ) + √(1−t) ∑ᵢ σᵢ z_{i,α} + ∑ᵢ hᵢ σᵢ` of
...
`√t H_N(σ) + √(1−t) ∑ᵢ σᵢ z_{i,α} + ∑ᵢ hᵢ σᵢ`. -/
def branchHam (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z : MarksSpace N k) (α : Fin k → ℕ × ℕ)
    (σ : Config N) : ℝ :=
  Real.sqrt t * H σ + Real.sqrt (1 - t) * ∑ i, isingSpin (σ i) * branchMark N k z α i
    + ∑ i, h i * isingSpin (σ i)

lemma truncHam_apply {M : ℕ} (t : ℝ) (h : Fin N → ℝ) (ω : EnergySpace N × MarksSpace N k)
...
def branchZ (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z : MarksSpace N k) (α : Fin k → ℕ × ℕ) : ℝ :=
  ∑ σ : Config N, Real.exp (-branchHam N k t h H z α σ)

lemma branchZ_pos (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z : MarksSpace N k) (α : Fin k → ℕ × ℕ) :
    0 < branchZ N k t h H z α :=
  Finset.sum_pos (fun _ _ => Real.exp_pos _) Finset.univ_nonempty

/-- The branch partition function is the partial partition function of `truncHam`. -/
lemma wCondZ_truncHam {M : ℕ} (t : ℝ) (h : Fin N → ℝ) (ω : EnergySpace N × MarksSpace N k)
    (α : TruncBranch k M) :
...
functions. -/
lemma wZ_truncHam {M : ℕ} (u : TruncBranch k M → ℝ) (t : ℝ) (h : Fin N → ℝ) (ω : EnergySpace N × MarksSpace N k) :
...
theorem sum_wGibbs_pair_eq {M : ℕ} (u : TruncBranch k M → ℝ) (t : ℝ) (h : Fin N → ℝ)
...
def branchHamX (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ)
    (σ : Config N) : ℝ :=
  Real.sqrt t * H σ + Real.sqrt (1 - t) * ∑ i, isingSpin (σ i) * (z₀ i + ∑ p, x p i)
    + ∑ i, h i * isingSpin (σ i)

lemma branchHam_eq (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z : MarksSpace N k) (α : Fin k → ℕ × ℕ)
    (σ : Config N) :
...
def branchZX (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) : ℝ :=
  ∑ σ : Config N, Real.exp (-branchHamX N k t h H z₀ x σ)

lemma branchZ_eq (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z : MarksSpace N k) (α : Fin k → ℕ × ℕ) :
    branchZ N k t h H z α = branchZX N k t h H z.1 (branchMarks k z.2 α) := by
  unfold branchZ branchZX
  simp_rw [branchHam_eq]

lemma branchZX_pos (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    0 < branchZX N k t h H z₀ x :=
  Finset.sum_pos (fun _ _ => Real.exp_pos _) Finset.univ_nonempty

/-- Joint continuity of the branch partition function in `(t, H, z₀, x)`. -/
lemma continuous_branchZX (h : Fin N → ℝ) :
...
lemma continuous_branchZX' (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) :
...
branch. -/
def hamG (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) : ℝ≥0∞ :=
  ENNReal.ofReal (branchZX N k t h H z₀ x)

lemma hamG_pos (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    0 < hamG N k t h H z₀ x :=
  ENNReal.ofReal_pos.2 (branchZX_pos N k t h H z₀ x)

lemma measurable_hamG (h : Fin N → ℝ) :
...
lemma measurable_hamG' (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) :
    Measurable (hamG N k t h H z₀) := by
  unfold hamG
  exact (continuous_branchZX' N k t h H z₀).measurable.ennreal_ofReal

/-- `F_{k+1}` of Talagrand's (14.81): `∑ᵢ log (2 cosh (h i + z_{i,0} + ∑ₚ x_{i,p}))`. -/
def coshF (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) : ℝ :=
  ∑ i, Real.log (2 * Real.cosh (h i + z₀ i + ∑ p, x p i))

/-- `exp F_{k+1} = ∏ᵢ 2 cosh (h i + z_{i,0} + ∑ₚ x_{i,p})`, in `ℝ≥0∞`. -/
def coshG (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) : ℝ≥0∞ :=
  ENNReal.ofReal (Real.exp (coshF N k h z₀ x))

lemma exp_coshF (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    Real.exp (coshF N k h z₀ x) = ∏ i, (2 * Real.cosh (h i + z₀ i + ∑ p, x p i)) := by
  rw [coshF, Real.exp_sum]
  exact Finset.prod_congr rfl fun i _ => Real.exp_log (by positivity)

lemma coshG_eq (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    coshG N k h z₀ x = ENNReal.ofReal (∏ i, (2 * Real.cosh (h i + z₀ i + ∑ p, x p i))) := by
  rw [coshG, exp_coshF]

lemma coshG_pos (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) : 0 < coshG N k h z₀ x :=
  ENNReal.ofReal_pos.2 (Real.exp_pos _)

lemma continuous_coshF (h : Fin N → ℝ) :
...
lemma continuous_coshF' (h : Fin N → ℝ) (z₀ : Fin N → ℝ) : Continuous (coshF N k h z₀) := by
  unfold coshF
...
lemma measurable_coshF (h : Fin N → ℝ) :
    Measurable fun q : (Fin N → ℝ) × (Fin k → Fin N → ℝ) => coshF N k h q.1 q.2 :=
  (continuous_coshF N k h).measurable

lemma measurable_coshF' (h : Fin N → ℝ) (z₀ : Fin N → ℝ) : Measurable (coshF N k h z₀) :=
  (continuous_coshF' N k h z₀).measurable

lemma measurable_coshG (h : Fin N → ℝ) :
...
lemma measurable_coshG' (h : Fin N → ℝ) (z₀ : Fin N → ℝ) : Measurable (coshG N k h z₀) := by
  unfold coshG
...
lemma ofReal_prod_two_cosh_treeMark {M : ℕ} (h : Fin N → ℝ) (z : MarksSpace N k) (α : TruncBranch k M) :
    ENNReal.ofReal (∏ i, (2 * Real.cosh (h i + treeMark N k M z α i)))
...
lemma branchZX_eq_sum (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    branchZX N k t h H z₀ x
      = ∑ σ : Config N, Real.exp (-(Real.sqrt t * H σ + Real.sqrt (1 - t)
          * ∑ i, isingSpin (σ i) * z₀ i + ∑ i, h i * isingSpin (σ i))
...
lemma hamG_eq_sum (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    hamG N k t h H z₀ x
...
lemma lintegral_hamG_ne_top (vs : Fin k → ℝ≥0) (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) :
    ∫⁻ x, hamG N k t h H z₀ x ∂Measure.pi (gaussianMarks N k vs) ≠ ∞ := by
  simp_rw [hamG_eq_sum]
  exact lintegral_sum_ofReal_exp_gaussianMarks N k vs _ _

lemma coshG_eq_sum (h : Fin N → ℝ) (z₀ : Fin N → ℝ) (x : Fin k → Fin N → ℝ) :
    coshG N k h z₀ x
  ...
  congr 1
  have h1 := sum_exp_sum_spin N fun i => h i + z₀ i + ∑ p, x p i
  simp only [spin, spinOf, exp_add_exp_neg_eq_two_cosh] at h1
  rw [← h1]
  refine Finset.sum_congr rfl fun σ _ => ?_
  congr 1
  have e1 : ∑ i, (h i + z₀ i + ∑ p, x p i) * isingSpin (σ i)
      = ∑ i, (h i + z₀ i) * isingSpin (σ i) + ∑ p, ∑ i, isingSpin (σ i) * x p i := by
    simp_rw [add_mul, Finset.sum_add_distrib, Finset.sum_mul]
    rw [Finset.sum_comm]
    simp_rw [mul_comm]
  exact e1

lemma lintegral_coshG_ne_top (vs : Fin k → ℝ≥0) (h : Fin N → ℝ) (z₀ : Fin N → ℝ) :
...
lemma cascadeRec_hamG_ne_top (ms : Fin k → ℝ) (vs : Fin k → ℝ≥0) (hpos : ∀ i, 0 < ms i)
    (hle : ∀ i, ms i ≤ 1) (t : ℝ) (h : Fin N → ℝ) (H : EnergySpace N) (z₀ : Fin N → ℝ) :
