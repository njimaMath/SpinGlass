...
`H(σ) = -(K(σ) + β*h*magnetization(σ))`, so that `exp(-H(σ)) = exp(K(σ) + β*h*magnetization(σ))`.
-/
noncomputable def skEnergy (K : EnergySpace N) : EnergySpace N :=
  (-1) • K + magnetic_field_vector (N := N) (-β * h)
...
lemma skZ_eq (K : EnergySpace N) :
    skZ (N := N) (β := β) (h := h) K
      = ∑ σ : Config N, Real.exp (K σ + (β * h) * magnetization N σ) := by
  classical
  -- Unfolding shows `-skEnergy = K + (β*h)*magnetization`.
...
lemma integrable_log_skZ (sk : SKDisorder (Ω := Ω) N β h) :
  ...
  have hEnergy :
    ...
    have h2 : Continuous fun _K : EnergySpace N =>
        magnetic_field_vector (N := N) (-β * h) := continuous_const
  ...
  let cN : ℝ :=
    Real.log (Fintype.card (Config N) : ℝ) + ‖magnetic_field_vector (N := N) (-β * h)‖
  ...
  have hEnergy_norm :
      ‖skEnergy (N := N) (β := β) (h := h) (sk.U ω)‖
        ≤ ‖sk.U ω‖ + ‖magnetic_field_vector (N := N) (-β * h)‖ := by
    simpa [skEnergy, neg_one_smul] using
      (norm_add_le (-(sk.U ω)) (magnetic_field_vector (N := N) (-β * h)))
  have hAbs' :
      |Real.log (skZ (N := N) (β := β) (h := h) (sk.U ω))|
        ≤ Real.log (Fintype.card (Config N) : ℝ) +
          (‖sk.U ω‖ + ‖magnetic_field_vector (N := N) (-β * h)‖) := by
...
private lemma Z_interpol_zero_factorizes (ω : Ω) :
      ...
      (∑ γ : Config (N + M),
          Real.exp
            ((K_block (N := N) (M := M) (β := β) (h := h) (skN := skN) (skM := skM) ω) γ +
              (β * h) * magnetization (N := N + M) γ))
        =
      ∑ p : Config N × Config M,
        Real.exp
          ((skN.U ω) p.1 + (β * h) * magnetization (N := N) p.1) *
            Real.exp ((skM.U ω) p.2 + (β * h) * magnetization (N := M) p.2) := by
        ...
        refine
          (Fintype.sum_congr
            ...
            (fun p : Config N × Config M =>
              Real.exp
                  ((skN.U ω) p.1 + (β * h) * magnetization (N := N) p.1) *
                Real.exp ((skM.U ω) p.2 + (β * h) * magnetization (N := M) p.2))
            ?_)
        intro p
        have hadd :
            ((skN.U ω) p.1 + (skM.U ω) p.2) +
              ...
              =
            ((skN.U ω) p.1 + (β * h) * magnetization (N := N) p.1) +
              ((skM.U ω) p.2 + (β * h) * magnetization (N := M) p.2) := by
  ...
  calc
    ...
    _ =
      (∑ α : Config N,
          Real.exp ((skN.U ω) α + (β * h) * magnetization (N := N) α)) *
        (∑ σ : Config M,
          Real.exp ((skM.U ω) σ + (β * h) * magnetization (N := M) σ)) := by
      calc
        (∑ p : Config N × Config M,
            Real.exp
                ((skN.U ω) p.1 + (β * h) * magnetization (N := N) p.1) *
          ...
          simpa using
            (Fintype.sum_mul_sum
              (fun α : Config N =>
                Real.exp ((skN.U ω) α + (β * h) * magnetization (N := N) α))
              (fun σ : Config M =>
                Real.exp ((skM.U ω) σ + (β * h) * magnetization (N := M) σ))).symm
    _ =
      skZ (N := N) (β := β) (h := h) (skN.U ω) *
        skZ (N := M) (β := β) (h := h) (skM.U ω) := by
...
lemma exp_magnetization_le (σ : Config N) :
    Real.exp ((β * h) * magnetization N σ) ≤ Real.exp (|β * h| * (N : ℝ)) := by
  have hle :
      (β * h) * magnetization N σ ≤ |β * h| * (N : ℝ) := by
    calc
      (β * h) * magnetization N σ
          ≤ |(β * h) * magnetization N σ| := le_abs_self _
      _ = |β * h| * |magnetization N σ| := by
            simp [abs_mul]
      _ ≤ |β * h| * (N : ℝ) := by
            exact mul_le_mul_of_nonneg_left (abs_magnetization_le (N := N) σ) (abs_nonneg (β * h))
